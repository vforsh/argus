...
`keydown` sends one `keyDown` (carrying the text for printable keys) and one `keyUp` — the same two events Chrome's other automation clients send, and the same the page sees from a real key press: one `keydown`, one `keypress`, one `input`. It does not set `nativeVirtualKeyCode`; that field means the _platform's_ key code (a Carbon keycode on macOS), and sending the Windows value there made Chrome 152 headless repeat the key thousands of times a second until the renderer stopped answering CDP.
