...
**None.** C2 closed last (see [Closed last: C2](#closed-last-c2) below); everything else in this section closed earlier — see [`tasks/code-quality-remaining.md`](tasks/code-quality-remaining.md) for the record of how, including three items that landed differently than planned. 19 commits, `028f1e1..HEAD`, +3079/−2401 across 204 files. Gate at the end: `npm run typecheck`, `npm run lint`, `npm run test:e2e` — 114 unit tests + 25 e2e files, 0 failures.
...
**C2 — `frame_snapshot` replacing forged CDP events** was deliberately declined until an e2e could drive a real extension, and that is exactly how it closed (`2bb2d1f..0fdf983`, 4 commits, +1594/−492). In order:

1. **Safety net first** (`9d7f0a6`): `e2e/extension-live.test.ts` boots a real Chromium with the unpacked extension and real native hosts, fully isolated — manifests in the temp profile's `NativeMessagingHosts/` (Chromium resolves user-level hosts under `--user-data-dir`), `ARGUS_HOME`/`ARGUS_REGISTRY_PATH` baked into wrappers. Six scenarios: attach, same-origin iframe selection, selection across a top navigation, cross-origin OOPIF selection, selection across reload (target recovery), child-session detach. Green on the **pre-redesign** tree with the new `pageNavigations` status counter (`2bb2d1f`) reading **2 per real navigation** — the forged-replay defect, measured. Harness notes that were not in the plan: branded Chrome 137+ ignores `--load-extension` (harness requires Chrome for Testing/Chromium and skips itself otherwise), and `--use-mock-keychain` is mandatory on macOS.
2. **Redesign** (`0fdf983`): the extension owns the frame table (`background/frame-table.ts`) and publishes deduplicated full-table `frame_snapshot` messages; all three synthetic `emitEvent` sites are gone; the watcher applies every snapshot through one idempotent path (`sources/extension-frame-snapshot.ts`) and pulls the table via `frame_snapshot_request` at bootstrap/recovery instead of calling `Page.getFrameTree` itself. `onPageNavigation` now fires only from the real top-frame event — the e2e asserts `=== 1` per navigation and per reload. `NATIVE_MESSAGING_PROTOCOL_VERSION` bumped to 2 (handshake rejects mismatched peers). `debugger-manager.ts` 573 → 491 LOC.

One bug the ordering caught before it ever ran on Chrome: applying a pulled snapshot in the awaiting caller (a later microtask) let a stale table land after newer real events and rematch selection onto the wrong frame — the recovery unit suite failed on it, and snapshots are now applied inside the session manager's message handler, in wire order.
