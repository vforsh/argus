...
const handleDomAdd = async (req: http.IncomingMessage, res: http.ServerResponse, options: HttpServerOptions): Promise<void> => {
	...
	try {
		const { allNodeIds } = await resolveDomSelectorMatches(options.cdpSession, payload.selector, true)
		...
		const { insertedCount } = await insertAdjacentHtml(options.cdpSession, {
			nodeIds,
			html: payload.html,
...
const selectDomAddNodeIds = (allNodeIds: number[], options: { all: boolean; nth?: number }): number[] => {
	if (options.all) {
		return allNodeIds
	}
