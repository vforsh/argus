...
export const handle: RouteHandler = async (req, res, _url, ctx) => {
	const payload = await readJsonBody<DomScrollToRequest>(req, res)
	if (!payload) {
		return
	}
	...
	emitRequest(ctx, res, 'dom/scroll-to')
	...
	try {
		...
		if (!hasSelector) {
			...
			const response: DomScrollToResponse = { ok: true, scrollX, scrollY }
		...
		if (allNodeIds.length === 0) {
			const response: DomScrollToResponse = { ok: true, matches: 0, scrolled: 0, scrollX: 0, scrollY: 0 }
			return respondJson(res, response)
		}
