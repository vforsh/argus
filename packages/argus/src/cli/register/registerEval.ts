...
export function registerEval(program: Command): void {
	const evalCmd = program
		...
		.option('--timeout <duration>', 'Eval timeout in milliseconds or duration syntax (e.g. 60000, 60s, 2m)')
		...
		.option('--iframe-timeout <duration>', 'Timeout for iframe postMessage response (default: 5000; accepts 5s, 1m)')
...
`,
	...
	program
		...
		.option('--no-await', 'Do not await promises')
		.option('--timeout <duration>', 'Per-eval timeout in milliseconds or duration syntax (e.g. 60000, 60s, 2m)')
		.option('--json', 'Output JSON for automation')
		.option('--no-return-by-value', 'Disable returnByValue (use preview)')
		.option('--no-fail-on-exception', 'Do not exit with code 1 when the evaluation throws')
		.option('--retry <n>', 'Retry failed evaluations up to N times')
