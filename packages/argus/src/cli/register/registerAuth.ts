...
export function registerAuth(program: Command): void {
	...
	const cookies = addCookieListOptions(
		auth.command('cookies').argument('[id]', 'Watcher id').description('Inspect and mutate browser cookies'),
		'Only include first-party cookies for the attached page origin',
	)
		.addHelpText(
			'after',
	...
	addCookieListOptions(cookies.command('list').alias('ls'), 'Only include first-party cookies for the attached page origin')
...
const addCookieListOptions = (command: Command, forOriginDescription: string): Command =>
	addCookieScopeOptions(command, forOriginDescription)
		.option('--domain <domain>', 'Filter cookies by domain suffix')
