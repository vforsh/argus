...
const setupSchema = async (cwd: string): Promise<string> => {
	const schemaPath = path.join(cwd, 'schemas', 'argus.config.schema.json')
	await fs.mkdir(path.dirname(schemaPath), { recursive: true })
	await fs.writeFile(schemaPath, JSON.stringify({ $schema: 'https://json-schema.org/draft/2020-12/schema' }))
	return schemaPath
}
...
test('config init creates default config', async (t) => {
	...
	const schemaPath = await setupSchema(tempDir)
	...
	assert.equal(parsed.$schema, schemaPath)
