...
Аудит: `CODE_QUALITY_AUDIT.md` → C2 (MAJOR). Предусловие всей работы — этап 0: e2e на живом Chrome с настоящим расширением. Без него редизайн не начинать.
...
## Этап 0 — e2e-страховка (отдельный PR, зелёный на текущем коде)
...
- **C. Навигация с выбранным iframe (ядро):** навигировать top page → iframe переизбран (новый frameId, hint-механика из `removeExtensionFrame`), `eval` по-прежнему в iframe, `pageNavigations` зафиксирован. На текущем коде счётчик даёт **2+** за навигацию (реальное событие + синтетический реплей через 150 мс) — этап 0 ассертит `>= 1` и логирует значение; точное `=== 1` затягивается на этапе 3.
...
Скрипт `npm run test:e2e:extension`; в дефолтный `test:e2e` не включать до стабильности (этап 4).

## Этап 1 — протокол (в одном PR с этапами 2–3)
...
## Этап 4 — затянуть e2e и включить в гейт

1. Сценарий C: `pageNavigations === 1` на одну навигацию (ловит и регресс к репле-шторму, и потерю настоящего события).
...
| PR  | Состав                                         | Объём                                                 |
| --- | ---------------------------------------------- | ----------------------------------------------------- |
| 1   | Этап 0: харнесс + сценарии + счётчик навигаций | ~600–800 LOC нового кода, 2–4 дня с отладкой headless |
| 2   | Этапы 1–3 (лочатся бампом протокола) + этап 4  | диф ~ +400/−350, debugger-manager худеет на ~40%      |

Риски: headless+native messaging (спайк в начале этапа 0); тайминги recovery на живом Chrome (митигируется F-сценарием и rerun'ами); бамп wire-протокола (release note, handshake уже защищает).
