# C2 — `frame_snapshot` вместо подделанных CDP-событий · **СДЕЛАНО** (`2bb2d1f..0fdf983`)

План выполнен целиком и в заявленном порядке. Что разошлось с планом — ниже; остальное сделано как написано.

**Гейт:** `npm run typecheck`, `npm run lint` (0 ошибок), `npm run test:e2e` (exit 0, 27 сьют), `extension-live` 5× подряд зелёный.

**Расхождения с планом, которые стоит помнить:**

1. **Chrome не годится вообще.** Branded Google Chrome 137+ игнорирует `--load-extension` — на нём расширение просто не загружается, и спайк молча не давал ни одного native-host'а. Харнес требует Chrome for Testing / Chromium (`ARGUS_E2E_CHROME_BIN` или автопоиск в кэше Playwright) и пропускает сьюту, если бинаря нет. Плюс `--use-mock-keychain` обязателен на macOS: без него CfT висит на промпте Keychain и не создаёт даже профиль.
2. **Матрица «источник → эффекты» из первого разбора не понадобилась** — ровно как и рассчитывали: `onPageNavigation` живёт только в обработчике настоящего top-frame события, поэтому подавлять нечего по построению.
3. **Найден баг, которого в плане не было.** Ответ на `frame_snapshot_request` нельзя применять в ожидающем вызывающем коде: это следующий микротаск, и к тому моменту уже обработаны настоящие `cdp_event`, пришедшие следом по трубе — устаревшая таблица сносила только что созданные фреймы, а selection мог перепривязаться не туда. Снапшоты применяются внутри обработчика сообщений `SessionManager`, в порядке провода; промис остался сигналом завершения. Поймал юнит-сьют recovery до того, как это доехало до Chrome.
4. **`Page.getFrameTree` со стороны watcher удалён полностью**, вместе с `collectFrameTree` и `CdpFrameTreeNode` в `extension-frame-state.ts` — их инвариант переехал в `extension-frame-snapshot.test.ts`.
5. **Дедуп у источника оказался тоньше**, чем «сравнить с прошлой отправкой»: настоящие CDP-события тоже меняют таблицу, и watcher их видит сам, поэтому они складываются в кэш публикаций (`updateStateFromEvent`) — иначе первый же снапшот после реального события дублировал бы уже известное состояние.
6. **Побочно:** `debugger-manager.ts` 573 → 491 LOC (cookie-блок вынесен в `tab-cookies.ts`, таблица фреймов — в `frame-table.ts`).

**Не сделано и осталось за скобками:** сценарий lease/traversal для google-sheets (см. секцию ниже) — харнес для него теперь есть, но это другая поверхность, не про бухгалтерию фреймов.

---

Аудит: `CODE_QUALITY_AUDIT.md` → C2 (MAJOR).

**Предусловие старта: сначала закрыть все группы из [`code-quality-remaining.md`](./code-quality-remaining.md), потом браться за C2.** Что это даёт помимо очерёдности: группа 3a (typecheck для `e2e/`) должна быть готова до написания харнесса — новый `e2e/extension-live.test.ts` пишется сразу под компилятором; группа 3b задаёт правило импортов для харнеса (только публичные входы пакетов, включая `installNativeHostsTo` — экспортировать с JSDoc); группа 3c (sheets test-mirrors) закрывается внутри C2-харнесса, как и помечено в её описании; группы 1 и 5 меняют watcher — редизайн стартует от стабилизированной базы, а не параллельно с ней.

Всё — один PR, но с жёстким внутренним порядком: сначала этап 0 (e2e на живом Chrome с настоящим расширением), прогнанный и зелёный на **немодифицированном** коде как baseline-коммит в этой же ветке, и только потом редизайн. Без работающего харнесса протокол не трогать.
